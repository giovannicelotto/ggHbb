...
for toyGenIdx in $(seq 0 $((npdf - 1))); do
        combine -M GenerateOnly -d datacardMulti$CATEGORY.txt  --setParameterRanges r=$rMin,$rMax   --freezeParameters pdfindex_$CATEGORY"_2016_13TeV" --setParameters pdfindex_$CATEGORY"_2016_13TeV"=$toyGenIdx -n ToysPdf$toyGenIdx"_"$ext"_cat"$CATEGORY -m 125   -t $nToys --redefineSignalPOIs rateZbb \
        --expectSignal $signalExpected --saveToys
done
#
#
#
...
#
/work/gcelotto/miniconda3/envs/myenv/bin/python3 /t3home/gcelotto/ggHbb/WSFit/scripts/generateAndFitEnvelope.py --cat $CATEGORY --signalExpected $signalExpected --npdf $npdf --POI rateZbb