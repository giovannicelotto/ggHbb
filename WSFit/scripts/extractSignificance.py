...
categories = [0,1,2,10,11,None,100,101,None]
significanceIdx = np.array([[2,1,0],[None,11,10], [None,100,101]])
...
# %%
signalYieldsMatrix = np.zeros((3,len(categories)//3))
dataYieldsMatrix = np.zeros((3,len(categories)//3))


#significanceMatrix = np.zeros((3,len(categories)//3))
for i in range(significanceIdx.shape[0]):
    for j in range(significanceIdx.shape[1]):
        idx = significanceIdx[i,j]
        if idx is None:
            signalYieldsMatrix[i, j] = np.nan
            dataYieldsMatrix[i,j] = np.nan
            continue
