...
config = {
    'categories': [0,10,100, 'Combined'],
    'categoryLabels':['Tight', 'Medium', 'Loose', 'Combined'],
    'colors': ['blue', 'green', 'orange', 'red'],
    #'nPDFs': 3,
    #'pdfLabels': ['Bern2', 'Expo1', 'PolExpo3'] #0
    #'pdfLabels': ['Bern2', 'Bern3', 'Expo1', 'PolExpo2'] #10
    #'pdfLabels': ['Bern2', 'Expo1', 'PolExpo2'] #100
}
...
for i,cat in enumerate(config['categories']):
    ...
    ax.errorbar(
        ...
        yerr=0,fmt='o',color=config['colors'][i],markersize=10,linewidth=4,alpha=0.4
    )
    ax.errorbar(
        x=rMin,
        ...
        yerr=0,fmt='o',color=config['colors'][i],markersize=10,linewidth=2,label=f'Category {config["categoryLabels"][i]}'
    )
