...
config = {
    'categories': [0,1,2,10,11,12 ,'Combined'],
    'categoryLabels':[
                        #r'NN$_{XXT}$ Btag$_{TT}$',
                        #r'NN$_{XT}$ Btag$_{TT}$', 
                        r'NN$_{T}$ Btag$_{TT}$',
                        r'NN$_{M}$ Btag$_{TT}$',
                        r'NN$_{L}$ Btag$_{TT}$',
...
for cat in config['categories']:
    ...
    deltaNLL_Discrete_cat_i.append(2*branches["deltaNLL"])
...
for i,cat in enumerate(config['categories']):
    ...
    ax.plot(r_Discrete_cat_i[i][1:],deltaNLL_Discrete_cat_i[i][1:], 'o', markersize=3, linestyle='-', linewidth=1, alpha=0.3, color=color)
    ...
    ax.errorbar(
        ...
        yerr=0,fmt='o',color=color,markersize=10,linewidth=4,alpha=0.2,
    )
    ax.errorbar(
        x=rMin,
        ...
        yerr=0,fmt='o',color=color,markersize=10,linewidth=2,label=f'Category {config["categoryLabels"][i]}' + " (r = %.2f$_{%.2f}^{+%.2f}$)"%(rMin, x_intersectionLeft1Sigma-rMin, x_intersectionRight1Sigma-rMin)
    )
