...
# %%
df = loadMultiParquet_v2(paths=[37], nMCs=-1, columns=None, returnFileNumberList=False, selectFileNumberList=None, returnNumEventsTotal=False, filters=getCommonFilters(btagWP="M", cutDijet=True, ttbarCR=False))[0]
...
# %%
pt_thr = np.linspace(0.1, 0.5, 101)
...
# %%
plt.close('all')
mask_mumu = (df.jet2_muon_pt>9) & (abs(df.jet2_muon_dxySig)>6.) 
...
for dr_t in dr_values:
    ...
    for pt_t in pt_thr:
        ...
        eff_pt.append(mask.sum() / len(df[mask_mumu]))

    ax.plot(pt_thr, eff_pt, label=r"$\Delta R^{thr} < {%.1f}$"%(dr_t)) 
    if dr_t == 0.4:
        eff_WP = np.array(eff_pt)[pt_thr>0.4][0]
        ax.scatter(pt_thr[pt_thr>=0.4][0], eff_WP, color='red')
        ax.hlines(eff_WP, pt_thr[0], pt_thr[-1], colors='red', linestyles='dashed')
ax.text(x=0.4, y=0.95, s="%.1f%%"%(eff_WP*100))
ax.set_xlabel(r"$|\Delta p_T|/p_T$ jet2-Quark2 with Muon")
ax.set_ylabel("Efficiency of Jet 2")
ax.set_xlim(pt_thr[0], pt_thr[-1])
