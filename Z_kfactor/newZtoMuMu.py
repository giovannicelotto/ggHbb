...
# 0.01 to normalize the percentage 3.3%
...
h_LHE_HT = Hist.new.Reg(35, 0, 800, name="LHE_HT", label="LHE $H_T$ [GeV]").Weight()
h_LHE_ET = Hist.new.Reg(35, 0, 800, name="LHE_ET", label="LHE $E_T$ [GeV]").Weight()
hPartons = Hist.new.Reg(7, 0, 7, name="nOutgoingPartons", label="LHE n Partons").Weight()
h_PT = Hist.new.Variable(bins, name="pt", label="LHE Z $p_T$ [GeV]").Weight()
...
ax[0].text(x=0.9, y=0.9-0.05, s=f"Entries : {len(pt_all)}\n Weighted Entries : {ak.sum(final_weights):.0f}", transform=ax[0].transAxes, ha='right', fontsize=14)
ax[0].legend()
ax[0].set_yscale('log')
...
ax[1].text(x=0.9, y=0.9-0.05, s=f"Entries : {len(nPartons_all)}\n Weighted Entries : {ak.sum(final_weights):.0f}", transform=ax[1].transAxes, ha='right', fontsize=14)
ax[1].text(x=0.9, y=0.8-0.05, s=f"Min NOutgoingParton = {ak.min(nPartons_all):.0f}", transform=ax[1].transAxes, ha='right', fontsize=14)



h_LHE_HT.fill(LHE_HT=np.clip(LHE_HT_all, h_LHE_HT.axes[0].edges[0], h_LHE_HT.axes[0].edges[-1]), weight=final_weights)
ax[2].text(x=0.9, y=0.9-0.05, s=f"Entries : {len(LHE_HT_all)}\n Weighted Entries : {ak.sum(final_weights):.0f}", transform=ax[2].transAxes, ha='right', fontsize=14)
...
ax[3].text(x=0.9, y=0.9-0.05, s=f"Entries : {len(LHE_ET_all)}\n Weighted Entries : {ak.sum(final_weights):.0f}", transform=ax[3].transAxes, ha='right', fontsize=14)
