...
class Trainer:
    ...
    def train(self, hp: dict, outFolder: str):
...
def main(rank: int, world_size: int, hp: dict,
         ...
         inFolder, outFolder, inputSubFolder, featuresForTraining, mass_bins):
    ...
    WtrainTensor = torch.tensor(Wtrain, dtype=torch.float32, device=rank).unsqueeze(1)
    ...
    Wval_tensor = torch.tensor(Wval, dtype=torch.float32, device=rank).unsqueeze(1)
    ...
    trainer = Trainer(nn1=nn1, nn2=nn2, train_data=traindataloader, val_data=val_dataloader,
                      ...
                      mass_bins=mass_bins)
    trainer.train(hp, outFolder=outFolder)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l", "--lambda_dcor", type=float, help="lambda for penalty term", default=905)
    ...
    featuresForTraining, columnsToRead = getFeatures(outFolder, massHypo=True, bin_center=hp["bin_center_bool"])
    ...
    mp.spawn(main, args=(world_size, hp, Xtrain, Ytrain, Wtrain, rWtrain, genMassTrain, dijetMassTrain, Xval, Yval, Wval, rWval, genMassVal, dijetMassVal, inFolder, outFolder, inputSubFolder, featuresForTraining, mass_bins), nprocs=world_size)
