...
parser.add_argument("-s", "--size", type=int, help="Number of events to crop training dataset", default=9999999999999)
...
inFolder, outFolder = getInfolderOutfolder(name = "%s_%d_%s"%(current_date, args.boosted, str(args.lambda_disco).replace('.', 'p') if args.lambda_disco>0 else str(args.lambda_shape).replace('.', 'p')), suffixResults='_mjjDisco/gateSV')
...
Xtrain = scale(Xtrain,featuresForTraining,  scalerName= outFolder + "/model/myScaler.pkl" ,fit=True, features_to_exclude=['jet1_has_sv', 'jet2_has_sv'])
...
model = ClassifierWithSV(
    input_dim_main=(len(featuresForTraining)-6),
...
for epoch in range(args.epochs):
    ...
    for batch in train_dataloader:
        ...
        x_main, jet1_sv, jet2_sv, Y_batch, W_batch, dijetMass_batch, mask_batch = batch
        ...
        predictions = model(x_main, jet1_sv, jet2_sv)
