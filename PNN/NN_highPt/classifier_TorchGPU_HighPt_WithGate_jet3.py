...
#### Define arguments
parser.add_argument("-l", "--lambda_disco", type=float, help="lambda value for disco", default=50.)
parser.add_argument("-e", "--epochs", type=int, help="number of epochs", default=500)
parser.add_argument("-s", "--size", type=int, help="Number of events to crop training dataset", default=999999999999)
...
inFolder, outFolder = getInfolderOutfolder(name = "%s_%d_%s"%(current_date, args.boosted, str(args.lambda_disco).replace('.', 'p') if args.lambda_disco>0 else str(args.lambda_shape).replace('.', 'p')), suffixResults='_mjjDisco/gateSV_jet3')
...
Xtrain = scale(Xtrain,featuresForTraining,  scalerName= outFolder + "/model/myScaler.pkl" ,fit=True, features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3'])
...
for epoch in range(args.epochs):
    ...
    for batch in train_dataloader:
        ...
        x_main, jet1_sv, jet2_sv, jet3_batch, Y_batch, W_batch, dijetMass_batch, mask_batch = batch
        ...
        predictions = model(x_main, jet1_sv, jet2_sv, jet3_batch)
