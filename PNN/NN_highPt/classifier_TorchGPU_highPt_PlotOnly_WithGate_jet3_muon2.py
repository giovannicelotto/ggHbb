...
# %%
results = {}
inFolder_, outFolder = getInfolderOutfolder(name = "%s_%d_%s"%(args.date, args.boosted, str(args.version).replace('.', 'p')), suffixResults='_mjjDisco/gateSV_jet3_muon2', createFolder=False)
...
Xtrain = scale(Xtrain,featuresForTraining,  scalerName= outFolder + "/model/myScaler.pkl" ,fit=False, features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3', 'jet2_has_muon'])
...
with torch.no_grad():  # No need to track gradients for inference
    YPredTrain = model(Xtrain_main, Xtrain_sv1, Xtrain_sv2, Xtrain_jet3, Xtrain_jet2_muon).numpy()
    YPredVal = model(Xval_main, Xval_sv1, Xval_sv2, Xval_jet3, Xval_jet2_muon).numpy()
# %%
print("Unscaling predictions...")
Xtrain = unscale(Xtrain, featuresForTraining=featuresForTraining, scalerName= outFolder + "/model/myScaler.pkl", features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3', 'jet2_has_muon'])
Xval = unscale(Xval, featuresForTraining=featuresForTraining,   scalerName =  outFolder + "/model/myScaler.pkl", features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3', 'jet2_has_muon'])
...
nEvents = 400
subTensor_main = Xtrain_main[(genMassTrain==0) | (genMassTrain==125)] [:nEvents]
subTensor_sv1 = Xtrain_sv1[(genMassTrain==0) | (genMassTrain==125)] [:nEvents]
subTensor_sv2 = Xtrain_sv2[(genMassTrain==0) | (genMassTrain==125)] [:nEvents]
subTensor_jet3 = Xtrain_jet3[(genMassTrain==0) | (genMassTrain==125)] [:nEvents]
subTensor_muon2 = Xtrain_jet2_muon[(genMassTrain==0) | (genMassTrain==125)] [:nEvents]
# %%
import torch.nn as nn
X_all = torch.cat([subTensor_main, subTensor_sv1, subTensor_sv2, subTensor_jet3, subTensor_muon2], dim=1)
class WrappedModel(nn.Module):
    def __init__(self, model, dim_main, dim_sv, dim_jet3, dim_muon2):
        super().__init__()
        self.model = model
        self.dim_main = dim_main
        self.dim_sv = dim_sv
        self.dim_jet3 = dim_jet3
        self.dim_muon2 = dim_muon2

    def forward(self, x):
        x_main = x[:, :self.dim_main]
        x_sv1  = x[:, self.dim_main:self.dim_main + self.dim_sv]
        x_sv2  = x[:, self.dim_main + self.dim_sv:self.dim_main + 2*self.dim_sv]
        x_jet3  = x[:, self.dim_main + 2*self.dim_sv:self.dim_main + 2*self.dim_sv+self.dim_jet3]
        x_muon2  = x[:, self.dim_main + 2*self.dim_sv+self.dim_jet3:self.dim_main + 2*self.dim_sv+self.dim_jet3+self.dim_muon2]

        return self.model(x_main, x_sv1, x_sv2, x_jet3, x_muon2)
dim_main = Xtrain_main.shape[1]
dim_sv   = Xtrain_sv1.shape[1]
dim_jet3   = Xtrain_jet3.shape[1]
dim_muon2   = Xtrain_jet2_muon.shape[1]

wrapped_model = WrappedModel(model, dim_main, dim_sv, dim_jet3, dim_muon2)
wrapped_model.eval()
# %%
...
jet2_muon_features__ = [
    'jet2_muon_pt_prime',
    'jet2_muon_eta_prime',
    'jet2_muon_phi_prime',
    'jet2_muon_dxySig',
]
featuresForTraining_main = [f for f in featuresForTraining if f not in jet1_sv_features__ + jet2_sv_features__ + jet3_features__ + jet2_muon_features__]
main_features = featuresForTraining_main  # without SV features


feature_names_all = main_features + sv1_features + sv2_features + jet3_features + jet2_muon_features
...
plot_shap_bar(phi_all, feature_names_all, top_n=46, out_file=outFolder+"/performance/shapMC.png")
