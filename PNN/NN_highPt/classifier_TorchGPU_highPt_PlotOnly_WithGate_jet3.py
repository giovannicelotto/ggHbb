...
parser.add_argument("-e", "--epoch", type=int, help="epoch", default=163)
...
# %%
results = {}
inFolder_, outFolder = getInfolderOutfolder(name = "%s_%d_%s"%(args.date, args.boosted, str(args.version).replace('.', 'p')), suffixResults='_mjjDisco/gateSV_jet3', createFolder=False)
...
Xtrain = scale(Xtrain,featuresForTraining,  scalerName= outFolder + "/model/myScaler.pkl" ,fit=False, features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3'])
Xval  = scale(Xval, featuresForTraining, scalerName= outFolder + "/model/myScaler.pkl" ,fit=False, features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3'])
...
main_features = [f for f in featuresForTraining if f not in sv1_features + sv2_features + jet3_features]
main_features+=['jet1_has_sv', 'jet2_has_sv', 'has_jet3']
...
with torch.no_grad():  # No need to track gradients for inference
    YPredTrain = model(Xtrain_main, Xtrain_sv1, Xtrain_sv2, Xtrain_jet3).numpy()
    YPredVal = model(Xval_main, Xval_sv1, Xval_sv2, Xval_jet3).numpy()
# %%
print("Unscaling predictions...")
Xtrain = unscale(Xtrain, featuresForTraining=featuresForTraining, scalerName= outFolder + "/model/myScaler.pkl", features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3'])
Xval = unscale(Xval, featuresForTraining=featuresForTraining,   scalerName =  outFolder + "/model/myScaler.pkl", features_to_exclude=['jet1_has_sv', 'jet2_has_sv', 'has_jet3'])
...
subTensor_jet3 = Xtrain_jet3[:nEvents]
# %%
import torch.nn as nn
X_all = torch.cat([subTensor_main, subTensor_sv1, subTensor_sv2, subTensor_jet3], dim=1)
class WrappedModel(nn.Module):
    def __init__(self, model, dim_main, dim_sv, dim_jet3):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        x_jet3  = x[:, self.dim_main + 2*self.dim_sv:]

        return self.model(x_main, x_sv1, x_sv2, x_jet3)
dim_main = Xtrain_main.shape[1]
dim_sv   = Xtrain_sv1.shape[1]
dim_jet3   = Xtrain_jet3.shape[1]

wrapped_model = WrappedModel(model, dim_main, dim_sv, dim_jet3)
wrapped_model.eval()
# %%
...
#main_features = [f for f in featuresForTraining if f not in sv1_features + sv2_features + jet3_features]
#main_features+=['jet1_has_sv', 'jet2_has_sv', 'has_jet3'] # add the gate features to the main features



feature_names_all = main_features + sv1_features + sv2_features + jet3_features
