    ...
    def train(self, hp: dict, outFolder: str, save: bool, modelName: str):
        ...
        # ______________________________________________________________________________
            for batch in self.train_data:
                ...
                X_batch, Y_batch, W_batch, dijetMass_batch, jet2_btagTight_batch, mask_batch = batch
...
def main(rank: int, world_size: int, hp: dict,
         X_train, Y_train, W_train, rW_train, genMass_train, dijetMassTrain,jet2_btagTightTrain,
         X_val, Y_val, W_val, rW_val, genMass_val, dijetMassVal,jet2_btagTightVal,
    ...
    jet2_btagTightTrain_tensor = torch.tensor(jet2_btagTightTrain, dtype=torch.float32, device=rank).unsqueeze(1)
    ...
    jet2_btagTightVal_tensor = torch.tensor(jet2_btagTightVal, dtype=torch.float32, device=rank).unsqueeze(1)
    ...
    jet2_btagTightTrain_tensor.to(rank),
    train_masks.to(rank)
    )
    ...
    val_dataset = TensorDataset(
        ...
        jet2_btagTightVal_tensor.to(rank),
        val_masks.to(rank)
    )
    ...
    set_seed(1)
    nn1 = Classifier(input_dim=X_train[featuresForTraining].shape[1], nNodes=hp["nNodes"])
    set_seed(2)
    nn2 = Classifier(input_dim=X_train[featuresForTraining].shape[1], nNodes=hp["nNodes"])
...
if __name__ == "__main__":
    ...
    #rW_train[Y_train==0]=rW_train[Y_train==0]*2
    #rW_val[Y_val==0]=rW_val[Y_val==0]*2

    dijetMassTrain = np.array(X_train.dijet_mass.values)
    dijetMassVal = np.array(X_val.dijet_mass.values)
    jet2_btagTightTrain = np.array(X_train.jet2_btagTight.values)
    jet2_btagTightVal = np.array(X_val.jet2_btagTight.values)
    ...
    size = hp['size']
    X_train, Y_train, W_train, rW_train, genMass_train, dijetMassTrain, jet2_btagTightTrain = X_train[:size], Y_train[:size], W_train[:size], rW_train[:size], genMass_train[:size], dijetMassTrain[:size], jet2_btagTightTrain[:size]
    X_val, Y_val, W_val, rW_val, genMass_val, dijetMassVal, jet2_btagTightVal = X_val[:size], Y_val[:size], W_val[:size], rW_val[:size], genMass_val[:size], dijetMassVal[:size], jet2_btagTightVal[:size]
    ...
    mp.spawn(main, args=(world_size, hp, X_train, Y_train, W_train, rW_train, genMass_train, dijetMassTrain, jet2_btagTightTrain, X_val, Y_val, W_val, rW_val, genMass_val, dijetMassVal, jet2_btagTightVal, inFolder, outFolder, inputSubFolder, featuresForTraining, mass_bins, args.saveResults, args.modelName), nprocs=world_size)
