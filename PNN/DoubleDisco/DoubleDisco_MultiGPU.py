...
def main(rank: int, world_size: int, hp: dict,
    ...
    set_seed(1)
    nn1 = Classifier(input_dim=Xtrain[featuresForTraining].shape[1], nNodes=hp["nNodes"], dropout_prob=0.1)
    set_seed(2)
    nn2 = Classifier(input_dim=Xtrain[featuresForTraining].shape[1], nNodes=hp["nNodes"], dropout_prob=0.1)
...
if __name__ == "__main__":
    ...
    featuresForTraining, columnsToRead = getFeatures(outFolder=outFolder, massHypo=False, bin_center=hp["bin_center_bool"], boosted=args.boosted, jet3_btagWP=True)
