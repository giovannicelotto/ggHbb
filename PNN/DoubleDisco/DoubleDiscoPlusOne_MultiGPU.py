    ...
    def train(self, hp: dict, outFolder: str, save: bool, modelName: str):
        ...
        # ______________________________________________________________________________
            for batch in self.train_data:
                ...
                # Combined loss
                loss = classifier_loss1 +classifier_loss2 + 1/(len(self.mass_bins) -1) * (hp["lambda_dcor"] * dCorr_total + 300*(distance_corr(bin_predictions1, jet2_btagTight_batch[bin_mask], bin_weights) + distance_corr( jet2_btagTight_batch[bin_mask], bin_predictions2, bin_weights)))
                loss.backward()
...
def main(rank: int, world_size: int, hp: dict,
         Xtrain, Ytrain, Wtrain, rWtrain, genMassTrain, dijetMassTrain,jet2_btagTightTrain,
         Xval, Yval, Wval, rWval, genMassVal, dijetMassVal,jet2_btagTightVal,
...
if __name__ == "__main__":
    ...
    jet2_btagTightTrain = np.array(Xtrain.jet2_btagTight.values)
    jet2_btagTightVal = np.array(Xval.jet2_btagTight.values)
    ...
    size = hp['size']
    Xtrain, Ytrain, Wtrain, rWtrain, genMassTrain, dijetMassTrain, jet2_btagTightTrain = Xtrain[:size], Ytrain[:size], Wtrain[:size], rWtrain[:size], genMassTrain[:size], dijetMassTrain[:size], jet2_btagTightTrain[:size]
    Xval, Yval, Wval, rWval, genMassVal, dijetMassVal, jet2_btagTightVal = Xval[:size], Yval[:size], Wval[:size], rWval[:size], genMassVal[:size], dijetMassVal[:size], jet2_btagTightVal[:size]
    ...
    mp.spawn(main, args=(world_size, hp, Xtrain, Ytrain, Wtrain, rWtrain, genMassTrain, dijetMassTrain, jet2_btagTightTrain, Xval, Yval, Wval, rWval, genMassVal, dijetMassVal, jet2_btagTightVal, inFolder, outFolder, inputSubFolder, featuresForTraining, mass_bins, args.saveResults, args.modelName), nprocs=world_size)
