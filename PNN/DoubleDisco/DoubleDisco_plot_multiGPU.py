...
inFolder, outFolder = getInfolderOutfolder(name = "%s_%s"%(current_date, str(hp["lambda_dcor"]).replace('.', 'p')), suffixResults='DoubleDisco', createFolder=False)
...
while True:
    ...
    train_closure_loss_history_rank = np.load(outFolder + "/model/train_closure_loss_history_rank%d.npy"%(rank))
    val_closure_loss_history_rank = np.load(outFolder + "/model/val_closure_loss_history_rank%d.npy"%(rank))
    if rank == 0:
        ...
        train_closure_loss_history = train_closure_loss_history_rank
        val_closure_loss_history = val_closure_loss_history_rank
    else:
        ...
        train_closure_loss_history = train_closure_loss_history + train_closure_loss_history_rank
        val_closure_loss_history = val_closure_loss_history + val_closure_loss_history_rank
    rank += 1
nRanks = rank
...
nn1_t = 0.5#(max_nn1 + secondMax_nn1)/2
nn2_t = 0.5#(max_nn2 + secondMax_nn2)/2
print("NN1_t", nn1_t)
print("NN2_t", nn2_t)
