...
# %%
results = {}
inFolder, outFolder = getInfolderOutfolder(name = "%s_%s"%(current_date, str(hp["lambda_reg"]).replace('.', 'p')), suffixResults='TripleDisco')
modelName1, modelName2, modelName3 = "nn1.pth", "nn2.pth", "nn3.pth"
...
nn3 = torch.load(outFolder+"/model/%s"%modelName3, map_location=torch.device('cpu'))
nn1.eval()
nn2.eval()
nn3.eval()
with open(outFolder+"/model/model_summary.txt", "w") as f:
    ...
    f.write("\n\nLayer-wise Details:\n")
    for name, module in nn2.named_modules():
        f.write(f"Layer: {name}\n")
        f.write(f"  Type: {type(module)}\n")
        if hasattr(module, 'weight') and isinstance(module.weight, torch.Tensor):
            f.write(f"  Weight Shape: {tuple(module.weight.shape)}\n")
...
# Classifier 3
cS = np.histogram(Xval[Yval==0].PNN3,bins=bins)[0]
cD = np.histogram(Xval[Yval==1].PNN3,bins=bins)[0]
cS=cS/np.sum(cS)
cD = cD/np.sum(cD)

ax[2].hist(bins[:-1], bins=bins, weights=cS, histtype='step', color='blue') 
ax[2].hist(bins[:-1], bins=bins, weights=cD, histtype='step', color='red') 
ax[2].set_xlabel("Classifier 3")
...
auc_vs_m(Ytrain, Yval, YPredTrain2, YPredVal2, genMassTrain, genMassVal, outFile=outFolder+"/performance/auc3_vs_m.png")
plt.close('all')
# %%
...
fpr_train, tpr_train, _ = roc_curve(Ytrain[(genMassTrain==125) | (genMassTrain==0)], YPredTrain3[(genMassTrain==125) | (genMassTrain==0)])
roc_auc_train = auc(fpr_train, tpr_train)
fpr_test, tpr_test, _ = roc_curve(Yval[(genMassVal==125) | (genMassVal==0)], YPredVal3[(genMassVal==125) | (genMassVal==0)])
roc3_auc_test = auc(fpr_test, tpr_test)
results["roc_125"] = (roc1_auc_test + roc2_auc_test + roc3_auc_test)/3
ax.plot(fpr_train, tpr_train, color='blue', lw=2, label=f'Train ROC curve (AUC = {roc_auc_train:.2f})')
ax.plot(fpr_test, tpr_test, color='red', lw=2, label=f'Validation ROC curve (AUC = {roc2_auc_test:.2f})')
ax.plot([0, 1], [0, 1], color='green', linestyle='--', lw=1)
ax.set_xlabel('False Positive Rate')
...
ax.legend()
ax.grid(alpha=0.3)
fig.savefig(outFolder+"/performance/ROC3_125.png")
plt.close('all')
# %%
...
hB_ADC = plot4ABCD(regions, mass_bins, x1, x2, nn2_t, nn1_t, suffix='temp', blindPar=(False, 125, 20), outName=outFolder+"/performance/abcd_check4R", sameWidth=False)
qcd_mc = regions['B']
print(qcd_mc.values())
print(hB_ADC.values())
QCD_SR(mass_bins, hB_ADC, qcd_mc, suffix="temp", blindPar=(False, 125, 10), outName=outFolder+"/performance/abcd_checkSR", sameWidth=False)
...
regions['C'].fill(df[mC][xx])
regions['D'].fill(df[mD][xx])
hB_ADC = plot4ABCD(regions, mass_bins, x1, x2, nn2_t, nn1_t, suffix='temp', blindPar=(False, 125, 20), outName=outFolder+"/performance/abcd_check4R_test", sameWidth=False)
qcd_mc = regions['B']
print(qcd_mc.values())
print(hB_ADC.values())
QCD_SR(mass_bins, hB_ADC, qcd_mc, suffix="temp", blindPar=(False, 125, 10), outName=outFolder+"/performance/abcd_checkSR_test", sameWidth=False)
...
getShapTorch(Xtest=Xtest[featuresForTraining].head(nEvents), model=nn1, outName=outFolder+'/performance/shap1.png', nFeatures=15, class_names=['NN1 output'], tensor=X_tensor_test)
getShapTorch(Xtest=Xtest[featuresForTraining].head(nEvents), model=nn2, outName=outFolder+'/performance/shap2.png', nFeatures=15, class_names=['NN2 output'], tensor=X_tensor_test)
