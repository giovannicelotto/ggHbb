...
parser.add_argument("-s", "--size", type=int, help="Number of events to crop training dataset", default=10000)
parser.add_argument("-b", "--boosted", type=int, help="Boosted Class", default=4)
parser.add_argument("-d", "--dropout", type=float, help="dropout prob", default=0.1)
parser.add_argument("-lr", "--learningRate", type=float, help="learning rate", default=0.001)
parser.add_argument("-bs", "--batch_size", type=int, help="Number of events perBatch", default=256)
parser.add_argument("-eS", "--earlyStopping", type=int, help="patience early stop", default=30)
parser.add_argument("-n", "--nodes",type=lambda s: [int(item) for item in s.split(',')],  # Convert comma-separated string to list of ints
                        help="List of nodes per layer (e.g., 128,64,32 for a 3-layer NN)",default='16,16'
)
...
outFolder = "/t3home/gcelotto/ggHbb/PNN/NN_FitScore/results/%s_%d_%s"%(current_date, boosted, str(args.version).replace('.', 'p'))
...
# ______________________________________________________________________________
    ...
    if (patience_counter >= args.earlyStopping) & (epoch > args.earlyStopping):
