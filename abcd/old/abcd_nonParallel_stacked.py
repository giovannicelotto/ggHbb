...
def getProcessesDataFrame():
    ...
    processes = {
        #'BParkingDataRun20181A':            [nanoPathCommon + "/Data1A2024Mar05/ParkingBPH1",                                                                                flatPathCommon + "/Data1A"    ,                   -1],
        ...
        #'TTToHadronic':                     [nanoPathCommon + "/ttbar2024Feb14/TTToHadronic_TuneCP5_13TeV-powheg-pythia8",                                            flatPathCommon + "/ttbar/ttbarHadronic"         , 831*0.45],
        #'TTToSemiLeptonic':                 [nanoPathCommon + "/ttbar2024Feb14/TTToSemiLeptonic_TuneCP5_13TeV-powheg-pythia8",                                        flatPathCommon + "/ttbar/ttbarSemiLeptonic"     , 831*0.46],
...
def main(nFilesData, nFilesMC):
    ...
    for (process, flatPath, xsection) in zip(df.index, df.flatPath, df.xsection):
        ...
        for flatFileName in flatFileNames:
            ...
            df = pd.read_parquet(flatFileName, columns=['sf', 'jet1_btagDeepFlavB', 'jet2_btagDeepFlavB', 'dijet_twist', 'dijet_mass', 'jet2_eta'])
    ...
    ax[0,1].hist(bins[:-1], bins=bins, weights=aCounts*dCounts/cCounts, histtype=u'step', label=r'$A\times D / C$ ', color='black', linewidth=2, linestyle='dotted')
    ax[0,2].hist(bins[:-1], bins=bins, weights=bCounts*cCounts/(aCounts*dCounts), histtype=u'step', label=r'B$\times$C/A$\times$D')
    ...
    for i in range(len(bins)-1):
        ...
        rect = patches.Rectangle(   (bins[i], bCounts[i]*cCounts[i]/(aCounts[i]*dCounts[i]) - sigmaR[i]),
