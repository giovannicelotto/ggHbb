...
import sys
hep.style.use("CMS")
def plot4ABCD(regions, bins, x1, x2, t1, t2, suffix, blindPar):
...
def SM_SR(regions, hB_ADC, bins, dfsData, dfsMC, isMCList, dfProcesses, x1, t1, x2, t2, lumi, suffix, blindPar):
    ...
    ax[0].errorbar(x, hExcess.values()*blind_mask, yerr=np.sqrt(hExcess.variances())*blind_mask , marker='o', color='black', linestyle='none')
