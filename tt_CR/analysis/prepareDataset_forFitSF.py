...
# id_to_process used to go from processNumber to processName
id_to_process = {}
...
#%%
# Assign to dataframe the processName
for idx, (mc_id, df) in enumerate(zip(np.array(isMCList)[np.array(nMCs) != 0], dfsMC)):
...
# Apply the cuts
dfData = dfData.query(config_cuts["cuts_string"].replace("PNN", "PNN_qm"))
dfMC = dfMC.query(config_cuts["cuts_string"].replace("PNN", "PNN_qm"))
