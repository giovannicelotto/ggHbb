...
##############################
        ...
        if selected2==999:
            assert False
        #Jet_breg2018 has to be applied on mass as well : https://twiki.cern.ch/twiki/bin/viewauth/CMS/HiggsWG/BJetRegression
        ...
        if Jet_pt[selected1]>=20:
            if Jet_btagDeepFlavB[selected1] < 0.049:
                jet1_btagWP = 0
            elif Jet_btagDeepFlavB[selected1] < 0.2783:
                jet1_btagWP = 1
            elif Jet_btagDeepFlavB[selected1] < 0.71:
                jet1_btagWP = 2
            else:
                jet1_btagWP = 3
        else:
        ...
        features_['jet1_btagWP'] = int(jet1_btagWP)
        ...
        if Jet_pt[selected2]>=20:
            if Jet_btagDeepFlavB[selected2] < 0.049:
                jet2_btagWP = 0
            elif Jet_btagDeepFlavB[selected2] < 0.2783:
                jet2_btagWP = 1
            elif Jet_btagDeepFlavB[selected2] < 0.71:
                jet2_btagWP = 2
            else:
                jet2_btagWP = 3
        else:
        ...
        features_['jet2_btagWP'] = int(jet2_btagWP)
...
# Dijet
        ...
        if isMC==0:
            ...
            if np.abs(branches["dijet_pt"][ev]-dijet.Pt())>1:
                print("%.3f vs %.3f"%(branches["dijet_pt"][ev], dijet.Pt()))

                print("%.3f vs %.3f"%(branches["dijet_jet1_pt"][ev], jet1.Pt()))
                print("%.3f vs %.3f"%(branches["dijet_jet2_pt"][ev], jet2.Pt()))
                print(Jet_pt*Jet_bReg2018)
                print(Jet_btagDeepFlavB)
                print(Jet_jetId)
                print(Jet_puId)
...
# Trig Muon
        ...
        if muonIdx2 != 999:
            ...
            features_['lepton2_pt'] = np.float32(muon2.Pt())
            features_['lepton2_eta'] = np.float32(muon2.Eta())
            features_['lepton2_phi'] = np.float32(muon2.Phi())
            features_['lepton2_dxySig'] = np.float32(Muon_dxy[muonIdx2]/Muon_dxyErr[muonIdx2])
            features_['lepton2_dzSig'] = np.float32(Muon_dz[muonIdx2]/Muon_dzErr[muonIdx2])
            features_['lepton2_IP3d'] = np.float32(Muon_ip3d[muonIdx2])
            features_['lepton2_sIP3d'] = np.float32(Muon_sip3d[muonIdx2])
            features_['lepton2_qualityId'] = bool(Muon_tightId[muonIdx2])
            #features_['lepton2_pfRelIso03_all'] = np.float32(Muon_pfRelIso03_all[muonIdx2])
            #features_['lepton2_pfRelIso04_all'] = np.float32(Muon_pfRelIso04_all[muonIdx2])
            features_['lepton2_tkIsoId'] = int(Muon_tkIsoId[muonIdx2])
            features_['lepton2_charge'] = int(Muon_charge[muonIdx2])
            features_['lepton2_pdgId'] = int(13)
            features_['dilepton_mass'] = np.float32((muon+muon2).M())
        else:
            # R2 or R3
            ...
            for mu in range(nMuon):
                ...
                if (mu != Jet_muonIdx1[selected2]) & (mu != Jet_muonIdx2[selected2]):
                    ...
                    features_['lepton2_eta'] = np.float32(muon2.Eta())
                    features_['lepton2_phi'] = np.float32(muon2.Phi())
                    features_['lepton2_dxySig'] = np.float32(Muon_dxy[mu]/Muon_dxyErr[mu])
                    features_['lepton2_dzSig'] = np.float32(Muon_dz[mu]/Muon_dzErr[mu])
                    features_['lepton2_IP3d'] = np.float32(Muon_ip3d[mu])
                    features_['lepton2_sIP3d'] = np.float32(Muon_sip3d[mu])
                    features_['lepton2_qualityId'] = bool(Muon_tightId[mu])
                    features_['lepton2_pfRelIso03_all'] = np.float32(Muon_pfRelIso03_all[mu])
                    #features_['lepton2_pfRelIso04_all'] = np.float32(Muon_pfRelIso04_all[mu])
                    #features_['lepton2_tkIsoId'] = int(int(Muon_tkIsoId[mu]))
                    features_['lepton2_charge'] = int(Muon_charge[mu])
                    features_['lepton2_pdgId'] = int(13)
                    features_['dilepton_mass'] = np.float32((muon+muon2).M())
                    break
        # R3
        if (Jet_nElectrons[selected2]>0) & (Electron_cutBased[Jet_electronIdx1[selected2]]>0):
                    leptonClass=2
                    lepton2 = ROOT.TLorentzVector(0., 0., 0., 0.)
                    electron_idx = Jet_electronIdx1[selected2]
                    lepton2.SetPtEtaPhiM(Electron_pt[Jet_electronIdx1[selected2]],
                                       Electron_eta[Jet_electronIdx1[selected2]],
                                       Electron_phi[Jet_electronIdx1[selected2]],
                                       0.511*10**(-3))
                    features_['leptonClass'] = int(leptonClass)
                    #features_["muon2Trig_RECO_ID"] = np.float32(1)
                    features_['lepton2_pt'] = np.float32(lepton2.Pt())
                    features_['lepton2_eta'] = np.float32(lepton2.Eta())
                    features_['lepton2_phi'] = np.float32(lepton2.Phi())
                    features_['lepton2_dxySig'] = np.float32(Electron_dxy[electron_idx]/Electron_dxyErr[electron_idx])
                    features_['lepton2_dzSig'] = np.float32(Electron_dz[electron_idx]/Electron_dzErr[electron_idx])
                    features_['lepton2_IP3d'] = np.float32(Electron_ip3d[electron_idx])
                    features_['lepton2_sIP3d'] = np.float32(Electron_sip3d[electron_idx])
                    features_['lepton2_qualityId'] = bool(Electron_cutBased[electron_idx])
                    features_['lepton2_pfRelIso03_all'] = np.float32(Electron_pfRelIso03_all[electron_idx])
                    #features_['lepton2_pfRelIso04_all'] = np.float32(Electron_pfRelIso04_all[electron_idx])
                    #features_['lepton2_tkIsoId'] = int(int(Electron_tkIsoId[electron_idx]))
                    features_['lepton2_charge'] = int(Electron_charge[electron_idx])
                    features_['lepton2_pdgId'] = int(11)
                    features_['dilepton_mass'] = np.float32((muon+lepton2).M())
                    
        elif leptonClass == 3:

            features_['leptonClass'] = int(leptonClass)
            #features_["muon2Trig_RECO_ID"] = np.float32(1)
            features_['lepton2_pt'] = np.float32(0)
            features_['lepton2_eta'] = np.float32(0)
            features_['lepton2_phi'] = np.float32(0)
            features_['lepton2_dxySig'] = np.float32(0)
            features_['lepton2_dzSig'] = np.float32(0)
            features_['lepton2_IP3d'] = np.float32(0)
            features_['lepton2_sIP3d'] = np.float32(0)
            features_['lepton2_qualityId'] = bool(False)
            features_['lepton2_pfRelIso03_all'] = np.float32(0)
            features_['lepton2_pfRelIso04_all'] = np.float32(0)
            features_['lepton2_tkIsoId'] = int(0)
            features_['lepton2_charge'] = int(0)
            features_['lepton2_pdgId'] = int(0)
            features_['dilepton_mass'] = np.float32(0.106)
