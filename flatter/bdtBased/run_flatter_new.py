...
def main(isMC, nFiles, maxEntries):
    ...
    for nanoFileName in nanoFileNames:
        ...
        print(flatPath)
        subprocess.run(['sbatch', '-J', process+"%d"%random.randint(1, 40), '/t3home/gcelotto/ggHbb/flatter/job_new.sh', nanoFileName, str(maxEntries), str(isMC), process, str(fileNumber), flatPath])
