...
def treeFlatten(fileName, maxEntries, maxJet, pN, processName, method, isJEC, verbose, JECname, isMC, folder_cfg, fileNumber):
    ...
    print("[DEBUG] Opening file with uproot", flush=True)
    f = uproot.open(fileName)
    print("[DEBUG] Opening tree", flush=True)
    tree = f['Events']
    print("[DEBUG] Getting branches", flush=True)
    branches_to_load = [
        "event",
        "run",
        "luminosityBlock",
    ...
    if isMC:
        branches_to_load += ["Muon_genPartIdx", "Jet_hadronFlavour", "Pileup_nTrueInt","genWeight"]
...
#
##############################
        evt = get_event_branches(branches, ev,isMC, run=3)
...
##############################
        event_weight  = 1.0
        evt["Jet_jetId"] = np.ones(len(evt["Jet_pt"]), dtype=int)*6
        evt["Jet_puId"] = np.ones(len(evt["Jet_pt"]), dtype=int)*6
        evt["Muon_isTriggering"] = np.ones(len(evt["Muon_pt"]), dtype=int) * (evt["Muon_pt"]>9) * (np.abs(evt["Muon_dxy"]/evt["Muon_dxyErr"])>6) * (np.abs(evt["Muon_eta"])<0.88) * (evt["HLT_Mu10_Barrel_L1HP11_IP6"]==1)
        evt["Jet_bReg2018"] = np.ones(len(evt["Jet_pt"]))
        evt["Jet_leadTrackPt"] = np.ones(len(evt["Jet_pt"]))
        evt["Jet_vtx3dL"] = np.ones(len(evt["Jet_pt"]))
        evt["Jet_vtx3deL"] = np.ones(len(evt["Jet_pt"]))
        evt["Jet_vtxPt"] = np.ones(len(evt["Jet_pt"]))
        evt["Jet_vtxMass"] = np.ones(len(evt["Jet_pt"]))
        evt["Jet_vtxNtrk"] = np.ones(len(evt["Jet_pt"]))
        maskJets =  (evt["Jet_pt"]>=20) & (abs(evt["Jet_eta"])<2.5)
        ...
        # Dijet
        dijet_features = fill_dijet_features(dijet, jet1, jet2, evt, run=3)
...
def main(fileName, maxEntries, maxJet, pN, fullProcessName, method, isJEC, verbose, isMC, fileNumber):
    ...
    data_predictions1, data_predictions_pca_varied = predict(df, "Jan21_3_50p0", boosted=3, run=3)
    df['PNN'] = data_predictions1.reshape(-1)
    df['PNN_pca'] = data_predictions_pca_varied.reshape(-1)
