...
import random
def bdtJetSelector(Jet_pt, Jet_eta, Jet_phi, Jet_mass, Jet_btagDeepFlavB, Jet_qgl, Jet_nTrigMuons, bst_loaded, isMC):
    ...
    for (i, j) in jet_indices:
        row = {
            ...
            'jet1_btagDeepFlavB': Jet_btagDeepFlavB[i], 'jet1_qgl': Jet_qgl[i], 'jet1_nTrigMuons': Jet_nTrigMuons[i],
            
            'jet2_pt': Jet_pt[j], 'jet2_eta': Jet_eta[j], 'jet2_phi': Jet_phi[j], 'jet2_mass': Jet_mass[j],
            'jet2_btagDeepFlavB': Jet_btagDeepFlavB[j], 'jet2_qgl': Jet_qgl[j], 'jet2_nTrigMuons': Jet_nTrigMuons[j],
