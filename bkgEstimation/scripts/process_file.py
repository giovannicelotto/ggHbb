...
def process_file(fileName, process):
    ...
    if process != 'Data':
        ...
        while True:
            ...
            try:
                numOld = np.load("/t3home/gcelotto/ggHbb/bkgEstimation/CountsBins/mini_%s.npy" % process)
                numUpdate = numOld + numEventsTotal
                np.save("/t3home/gcelotto/ggHbb/bkgEstimation/CountsBins/mini_%s.npy" % process, numUpdate)
    ...
    while True:
        lock_fd = acquire_lock()
        try:
            counts = np.histogram(np.clip(ht, bins[0], bins[-1]), bins=bins, weights=triggerSF if process!='Data' else None)[0]
            countsOld = np.load("/t3home/gcelotto/ggHbb/bkgEstimation/CountsBins/counts_%s.npy"%process)
            countsNew = countsOld + counts
            np.save("/t3home/gcelotto/ggHbb/bkgEstimation/CountsBins/counts_%s.npy"%process, countsNew)
            release_lock(lock_fd)
            break
        except:
            release_lock(lock_fd)
