...
from utilsForPlot import loadData, getFeaturesBScoreBased
...
def main():
    ...
    signal, realData = loadData(signalPath=signalPath, realDataPath=realDataPath, nSignalFiles=-1, nRealDataFiles=150)
    ...
    print("SIGNAL\n")
    signal, realData = cutSignalAndBackground(signal, realData, "Jet1Pt", 22, None, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "Jet1Eta", -5+2.46, 5-2.46, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "Jet1Mass", 6, None, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "Jet1_btagDeepFlavB", 0.2, None, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "Jet2Pt", 20, None, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "Jet2Eta", -2, 2, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "twist_Dijet", 0.76, None, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "Jet1Pt", 45, None, labels) # repeat for process understanding
    signal, realData = cutSignalAndBackground(signal, realData, "dEta_Dijet", 0, 1.4, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "Jet2_btagDeepFlavB", 0.35, 1, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "DijetMass", 60, 200, labels)
    signal, realData = cutSignalAndBackground(signal, realData, "ht", 180, None, labels)
    
    plotNormalizedFeatures(signal=signal, realData=realData, outFile = "/t3home/gcelotto/ggHbb/outputs/plots/normalizedFeatures_cuts.pdf")
    print("\n\n\n")
