...
def evaluateCriterion(maxJet, fileNames):
    ...
    for fileName in fileNames:
        ...
        for ev in  range(maxEntries):
            ...
            # Find the jets from the signal
                if (Jet_genJetIdx[i]>-1) & (abs(Jet_eta[i])<2.5):                                           # jet is matched to gen
    ...
    print("Events matched and within acceptance                         : %d  \t  %.2f" %(totalChoice, totalChoice/totalEntriesVisited*100))
    ...
    print("No dijets found with a trigger muon within all the jets      : %d  \t  %.2f" %(outOfConsideredJets , outOfConsideredJets/totalEntriesVisited*100))
    
    print("Consistency = nonMatched + correct + wrong + noDijetWithTrigger: %d  \t  %.1f" %(nonMatched+goodChoice+outOfConsideredJets+wrongChoice, (nonMatched+goodChoice+outOfConsideredJets+wrongChoice)/totalEntriesVisited))
...
def main(nFiles, maxJet1, maxJet2):
    ...
    for maxJet in range(maxJet1, maxJet2):
        ...
        evaluateCriterion(maxJet, fileNames)
