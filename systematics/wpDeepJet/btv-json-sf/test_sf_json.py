...
sfDir = os.path.join(".", "data", f"UL{opts.year}")
btvjson = correctionlib.CorrectionSet.from_file(os.path.join(sfDir, "btagging.json"))

# generate 20 dummy jet features
nJets     = 30
jet_pt    = np.random.exponential(50., nJets)
jet_eta   = np.random.uniform(0.0, 2.4, nJets)
jet_flav  = np.random.choice([0, 4, 5], nJets)
jet_discr = np.random.uniform(0.0, 1.0, nJets)
