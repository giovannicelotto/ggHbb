...
with open("/t3home/gcelotto/newFit/afterNN/cat1/config_1.yaml", "r") as f:
...
with open("/t3home/gcelotto/newFit/afterNN/cat1/fit_parameters_Z_cat1.json", "r") as f:
...
fig.savefig("/t3home/gcelotto/newFit/afterNN/cat1/plots/mjjFit_cat1.png", bbox_inches='tight')
# %%
...
ax[0].text(x=0.95, y=0.7, s="%d < Dijet p$_T$ < %d GeV\nJet1 bTag > %.2f\nJet2 bTag > %.2f\nNN score > %.1f\n"%(ptCut_min, ptCut_max, jet1_btagMin, jet2_btagMin, PNN_t), transform=ax[0].transAxes, ha='right', va='top', fontsize=24)
ax[0].tick_params(labelsize=24)
ax[1].tick_params(labelsize=24)
fig.savefig("/t3home/gcelotto/newFit/afterNN/cat1/plots/mjjFit2_cat1.png", bbox_inches='tight')
# End Plot 2
# %%
...
createRootHists([y_data_fit, y_Higgs, y_Z, y_data_blind], ['bkg', 'H', 'Z', 'data_obs'], bins=bins, outFolder="/t3home/gcelotto/newFit/afterNN/cat1/hists", suffix='cat1')
# %%
...
def amsc(s, b, breg):
    """
    Compute the Asimov Significance (AMSc).
    
    Parameters:
    s (float): Signal count
    b (float): Background count
    breg (float): Regularized background count
    
    Returns:
    float: The AMSc value
    """
    if b + breg <= 0:
        raise ValueError("Background and regularized background must be positive.")
    
    term = (s + b + breg) * np.log(1 + s / (b + breg)) - s
    return np.sqrt(2 * term) if term > 0 else 0.0
