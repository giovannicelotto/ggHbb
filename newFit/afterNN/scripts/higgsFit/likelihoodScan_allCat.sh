...
combineCards.py $dc_cat2p0 $dc_cat2p1 > $datacard
#Create ws
...
#combine -M MultiDimFit $ws --expectSignal 1 -t -1 --algo grid --points 100 --rMin $rmin --rMax $rmax --mass 125 --robustFit 1 -n hbb_JEC_Frozen.expected --freezeNuisanceGroups JEC
...
plot1DScan.py $outputCombine/higgsCombinehbb_total.expected.MultiDimFit.mH125.root \
  ...
  --others \
    ...
    $outputCombine/higgsCombinehbb_statOnly.expected.MultiDimFit.mH125.root:"Stat Only":2 \
  --output plots/HiggsExp_StatOnly \
  --breakdown "JER&JEC&btag,Bkg Estimation,Stat Only"\
  --pdf "y"
