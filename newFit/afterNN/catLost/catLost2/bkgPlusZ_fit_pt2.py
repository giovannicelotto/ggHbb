...
for idx, df in enumerate(dfsData):
    dfsData[idx] = df[~((df.jet1_btagDeepFlavB>jet1_btagMin) & (df.jet2_btagDeepFlavB>jet2_btagMin))]

for idx, df in enumerate(dfsMC_Z):
    dfsMC_Z[idx] = df[~((df.jet1_btagDeepFlavB>jet1_btagMin) & (df.jet2_btagDeepFlavB>jet2_btagMin))]

for idx, df in enumerate(dfsMC_H):
    dfsMC_H[idx] = df[~((df.jet1_btagDeepFlavB>jet1_btagMin) & (df.jet2_btagDeepFlavB>jet2_btagMin))]
...
ndof = m_tot_2.ndof
chi2_pvalue = 1- chi2.cdf(chi2_stat, ndof)
ax[0].text(x=0.05, y=0.65, s="$\chi^2$/ndof = %.1f/%d\np-value = %.3f"%(chi2_stat, ndof, chi2_pvalue), transform=ax[0].transAxes, ha='left')
ax[0].text(x=0.95, y=0.6, s="Dijet p$_T$ > %d GeV\nCat Lost\n"%(ptCut_min), transform=ax[0].transAxes, ha='right', va='top', fontsize=24)
ax[0].set_yscale('log')

fig.savefig(outFolder +"/plots/bkgPlusZPeak_160toInf.png", bbox_inches='tight')
# %%
...
fig.savefig(outFolder +"/plots/bkgPlusZPeak2_160toInf.png", bbox_inches='tight')
# End Plot 2
# %%
...
createRootHists([y_data_fit, y_Higgs, y_Z, y_data_blind], ['bkg', 'H', 'Z', 'data_obs'], bins=bins, outFolder=outFolder+"/hists", suffix='catLost2')
# %%
