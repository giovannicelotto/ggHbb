...
parser.add_argument('-c', '--cat', type=str, default="2", help='Config File')
...
with open("/t3home/gcelotto/ggHbb/WSFit/Configs/cat"+str(args.cat)+".yml", 'r') as f:
...
fit_systematics = FitWithSystematics(modelName, path, dfProcesses, x1, x2, outFolder, fitFunction, dfProcesses_sD=None, dfProcesses_sU=None, particle=args.particle)
# %%
...
for idx, df in enumerate(dfsMC):
    if df.dijet_mass.isna().sum()>0 or df.weight.isna().sum()>0:
        print("NaNs found in dijet_mass or weight for process ", dfProcesses.index[idx])
        RuntimeError("NaNs found")
...
fit_systematics.plot_results(x, cTot, err, m, fitregion, bins, outFolder, outName = args.particle+f"PeakFit_with_systematics_{args.cat}.png")
# %%
variations = [
    'btag_up', 'btag_down',
    ]
